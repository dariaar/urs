...
const ParalelnoPage = () => {
  ...
  if (user && user.email !== 'prof6@fesb.com') {
    return (
      <div style={{
  ...
  // Učitavanje studenata i prisutnosti za predmet "Paralelno programiranje"
  useEffect(() => {
    if (user && user.email === 'prof6@fesb.com') {
      const fetchStudents = async () => {
        try {
          ...
          const attendanceCollection = collection(db, 'class/Paralelno programiranje/students');
